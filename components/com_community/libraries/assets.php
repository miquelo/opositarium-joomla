    ...
    class CAssets {
        ...
        public function addVariable( $varname, $value ) {
            if ( !isset( $this->_variables ) ) {
                ...
                $mainframe->registerEvent('onBeforeCompileHead', array( $this, '_loadVariables' ));
            }
            $this->_variables[ $varname ] = $value;
        }

        public function _loadVariables() {
            $document = JFactory::getDocument();
            if ( isset( $this->_variables ) ) {
                $document->addScriptDeclaration('joms_vars = ' . json_encode( $this->_variables ) . ';');
            }
        }
