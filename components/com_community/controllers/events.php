...
class CommunityEventsController extends CommunityBaseController
{
    ...
    public function ajaxUnpublishEvent($eventId=null) {
        $mainframe = JFactory::getApplication();
        $jinput = $mainframe->input;
        $eventId = $jinput->post->get('eventid', '', 'INT');
        $response = new JAXResponse();
        ...
        if (!CFactory::getUser()->authorise('community.eventeditstate', 'com_community')) {
            ...
            if ($event->id == 0) {
                $response->addScriptCall('alert', JText::_('COM_COMMUNITY_EVENTS_ID_NOITEM'));
            } else {
                $event->published = 0;

                if ($event->store()) {
                    //trigger for onGroupDisable
                    $this->triggerGroupEvents('onGroupDisable', $event);
                    $mainframe->redirect(CRoute::_('index.php?option=com_community&view=events', false), JText::_('COM_COMMUNITY_EVENTS_UNPUBLISH_SUCCESS'));
                } else {
