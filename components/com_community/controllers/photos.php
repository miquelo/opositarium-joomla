    ...
    class CommunityPhotosController extends CommunityBaseController
    {
        ...
        public function ajaxSwitchPhotoTrigger($photoId, $showAll = false)
        {
            ...
            if (CAlbumsHelper::isFixedAlbum($album)) {
                ...
                if ($album->type == 'group') {
                    ...
                    $json['can_edit'] = (CFactory::getUser()->authorise('community.photoedit', 'com_community')) || ($album->creator == $my->id) || 
                        (
                            $album->groupid && ($isGroupAdmin || $isGroupOwner)
                        );
                    $json['can_delete'] = (CFactory::getUser()->authorise('community.photodelete', 'com_community')) || ($album->creator == $my->id) || 
                        (
                            $album->groupid && ($isGroupAdmin || $isGroupOwner)
                        );
                } elseif ($album->type == 'event') {
                    ...
                    $json['can_edit'] = (CFactory::getUser()->authorise('community.photoedit', 'com_community')) || ($album->creator == $my->id) || ($album->eventid && $isEventAdmin);
                    $json['can_delete'] = (CFactory::getUser()->authorise('community.photodelete', 'com_community')) || ($album->creator == $my->id) || ($album->eventid && $isEventAdmin);
                } else {
                    $json['can_edit'] = (CFactory::getUser()->authorise('community.photoedit', 'com_community')) || ($album->creator == $my->id);
                    $json['can_delete'] = (CFactory::getUser()->authorise('community.photodelete', 'com_community')) || ($album->creator == $my->id);
                }
        ...
        public function ajaxGetPhotosByAlbum($albumId, $photoId)
        {
            ...
            if (CFactory::getUser()->authorise('community.photodelete', 'com_community')) {
                ...
                $canDelete = ($album->hasAccess($my->id, 'deletephotos')  && CFactory::getUser()->authorise('community.photocreate', 'com_community'));
            }
